...
export default class AddNewComment extends Component {
  ...
  componentDidMount() {
    ...
    this.userCookie &&
      fetch(`http://localhost:3000/users/${this.userCookie.slice(7)}.json`)
        .then((res) => res.json())
        .then((data) =>
          this.setState({
  ...
  submitComment() {
    ...
    fetch(`http://localhost:3000/products/${this.props.productID}/comments.json`, {
      method: "POST",
  ...
  replayComment() {
    ...
    fetch(
      `http://localhost:3000/products/${this.props.productID}/comments/${this.props.replayID}/replay.json`,
      {
        method: "POST",
  ...
  render() {
    return (
      <div className="add-new-comment-wrapper">
        ...
        <button onClick={this.props.replayID ? this.replayComment : this.submitComment}>
          افزودن کامنت
        </button>
      </div>
    )
