...
export default class UserLoginStatus extends Component {
  ...
  componentDidMount() {
    ...
    if (this.userCookie.trim()) {
      fetch(`http://localhost:3000//users/${this.userCookie}.json`)
        .then((res) => res.json())
      ...
      <div className="topBar-register-box">
        <Link to={this.state.userData ? "/about-user" : "/login"}>
          <div className="topBar-register-datas">
            <span className="topBar-register-datas-user-name">
              {this.state.userData ? this.state.userData.userName : "حساب کاربری"}
            </span>
            <span className="topBar-register-datas-user-status">
              {this.state.userData ? this.state.userData.email : "لطفا وارد حساب خود شوید"}
            </span>
          </div>
