...
export default function AboutUser() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/users/${userId}/lastBuy.json`)
      .then((res) => res.json())
      .then((data) => setLastBuyList(data))

    fetch(`http://localhost:3000/users/${userId}/userName.json`)
      .then((res) => res.json())
  ...
  return (
    <>
      ...
      <div className="about-user-wrapper">
        <div className="head">
          <p>
            سلام {userName} عزیز! ({userName} نیستید؟ <b onClick={logOut}>خارج شوید</b>)
          </p>
          ...
          <>
            <Accordion key={cartProducts[0]} title={cartProducts[1].date}>
              <div className="old-buy-products">
                {Object.entries(cartProducts[1]).map(
                  (product) =>
                    product[0] !== "date" && (
                      <ProductCard
                        key={product[0]}
                        ...
                        img={product[1].gallery[0]}
                        title={product[1].productName}
                        price={product[1].price}
                        offPrice={product[1].offPrice}
                      />
                    )
                )}
              </div>
