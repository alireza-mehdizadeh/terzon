...
import { AiOutlineStar, AiOutlineBarChart, AiOutlineTags, AiOutlinePlusCircle, AiOutlineMinusCircle } from 'react-icons/ai'
...
export default function MainProduct() {
    ...
    useEffect(() => {
        ...
        fetch(`https://terzon-back.iran.liara.run/products/${params.productID}.json`)
            .then(res => res.json())
    ...
    const addToCart = () => {
        ...
        if (userId) {
            fetch(`https://terzon-back.iran.liara.run/users/${userId}/cart.json`, {
                method: 'POST',
    ...
    return (
        <>
                ...
                <div className="main-box">
                    ...
                    <div className="prosuct-more-datas">
                        ...
                        <div className="warranty-box">
                            ...
                            <p>محصول با گارانتی <span>{mainProduct.warranty}</span> تحویل داده میشود.</p>
                        </div>

                        <div className="delivery-box">
                            <CgSandClock className='icon' />
                            <p>حداکثر تا <span>{mainProduct.delivery}</span> تحویل داده می شود.</p>
                        </div>
                ...
                <div className="left-box">
                    <div className="add-to-cart-and-price-datas-box">
                        <ul>
                            ...
                            <li>
                                قیمت: <span className='price'>{mainProduct.price} تومان</span>
                            </li>
                            {mainProduct.offPrice && (
                                <li>
                                    قیمت با تخفیف: <span className='price'>{mainProduct.offPrice} تومان</span>
                                </li>
                            )}
                        </ul>
                        <div className="number-and-add-to-card-box">
                            <div className="product-number-to-buy">
                                <button onClick={minusProductCount}><AiOutlineMinusCircle /></button>
                                <span>{productCount}</span>
                                <button onClick={plusProductCount}><AiOutlinePlusCircle /></button>
                            </div>
                            <button disabled={addToCartBtn === 'افزودن به سبد خرید' ? false : true} className='add-to-card-btn' onClick={addToCart}>{addToCartBtn}</button>
                        </div>
                    </div>
            ...
            <div
                ...
                style={{ maxHeight: isShowMoreAbout ? 'unset' : "300px", paddingBottom: isShowMoreAbout ? '65px' : "0" }}
            >
                <h6>معرفی کوتاه</h6>
                ...
                <button onClick={showMoreAbout}>
                    <span>
                        {isShowMoreAbout ? 'دیدن کمتر...' : "دیدن بیشتر..."}
                    </span>
                </button>
            </div>
            ...
            <div className="product-comments">
                ...
                <div className="last-comments">
                    {
                        mainProduct.comments && Object.entries(mainProduct.comments).map(comment => (
                            <div key={comment[0]}>
                                <Comment commentID={comment[0]} name={comment[1].userName} text={comment[1].comment} onReplay={openReplayModal} />
                                {
                                    comment[1].replay && Object.entries(comment[1].replay).map(replayComment => (
                                        <ReplayComment key={replayComment[0]} commentID={comment[0]} name={replayComment[1].userName} text={replayComment[1].comment} onReplay={openReplayModal} />
                                    ))
                                }
                            </div>
                        ))
